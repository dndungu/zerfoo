...
// RecordMultiOutput appends a TracedOp for a multi-output operation like Split.
// Each output tensor gets its own slot index, stored in OutputIDs.
...
// RecordGather traces a Gather op where params is *TensorNumeric[T] and
// indices is *TensorNumeric[int]. The indices tensor is tracked by pointer
// identity using reflect, stored in the same slot namespace.
func (t *Tracer[T]) RecordGather(params *tensor.TensorNumeric[T], indices *tensor.TensorNumeric[int], output *tensor.TensorNumeric[T]) {
	paramsSlot := t.slotFor(params)
	indicesSlot := t.slotForIntTensor(indices)
	outSlot := t.slotFor(output)
	t.ops = append(t.ops, TracedOp{
		OpName:    "Gather",
		InputIDs:  []int{paramsSlot, indicesSlot},
		OutputID:  outSlot,
		ExtraArgs: nil,
	})
}
